...
func go_to_castle():
	...
	if is_second_level:
		run_to_castle_tween.tween_callback(Callable(self, "finish"))
	else:
		run_to_castle_tween.tween_callback(Callable(self, "switch_to_secondlevel"))

func _on_castle_area_body_entered(body):
	if body is Player:
		handle_castle_collision()

func handle_castle_collision():
	set_physics_process(false)
	z_index = -3
	var castle_tween = get_tree().create_tween()
	castle_tween.tween_property(self, "position", position + Vector2(0, 32), 1)
	castle_tween.tween_callback(switch_to_secondlevel)
